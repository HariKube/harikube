...
type Log interface {
	...
	List(ctx context.Context, prefix, startKey string, limit, revision int64, includeDeletes, keysOnly bool, labelSelector, fieldSelector string) (int64, []*server.Event, error)
...
func (l *LogStructured) get(ctx context.Context, key, rangeEnd string, limit, revision int64, includeDeletes, keysOnly bool, labelSelector, fieldSelector string) (int64, *server.Event, error) {
	rev, events, err := l.log.List(ctx, key, rangeEnd, limit, revision, includeDeletes, keysOnly, labelSelector, fieldSelector)
	if err != nil {
		return 0, nil, err
	}
	if revision != 0 {
...
func (l *LogStructured) List(ctx context.Context, prefix, startKey string, limit, revision int64, keysOnly bool, labelSelector, fieldSelector string) (revRet int64, kvRet []*server.KeyValue, errRet error) {
	...
	rev, events, err := l.log.List(ctx, prefix, startKey, limit, revision, false, keysOnly, labelSelector, fieldSelector)
	if err != nil {
		return rev, nil, err
	}

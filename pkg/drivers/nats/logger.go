...
func (b *BackendLogger) List(ctx context.Context, key, end string, limit, revision int64, keysOnly bool, labelSelector, fieldSelector string) (revRet int64, kvRet []*server.KeyValue, errRet error) {
	start := time.Now()
	defer func() {
		dur := time.Since(start)
	...
	return b.backend.List(ctx, key, end, limit, revision, keysOnly, labelSelector, fieldSelector)
}

// Count returns an exact count of the number of matching keys and the current revision of the database
func (b *BackendLogger) Count(ctx context.Context, key, end string, revision int64, labelSelector, fieldSelector string) (revRet int64, count int64, err error) {
	start := time.Now()
	defer func() {
		dur := time.Since(start)
	...
	return b.backend.Count(ctx, key, end, revision, labelSelector, fieldSelector)
}
...
func (b *BackendLogger) Watch(ctx context.Context, key, end string, revision int64, labelSelector, fieldSelector string) server.WatchResult {
	return b.backend.Watch(ctx, key, end, revision, labelSelector, fieldSelector)
}
...
func (b *BackendLogger) Transaction(ctx context.Context) (server.Transaction, error) {
	return b.backend.Transaction(ctx)
}
